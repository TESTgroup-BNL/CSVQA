...
if __name__ == "__main__":
    ...
    for fname in getFiles(fPath, opts["Level 1"]["altFileSuffix"]):
        ...
        #Load alt source devations if available
        if opts["Level 1"]["altCheckEnable"]:
            d_dev = loadData(fname_s[0] + "_dev" + fname_s[1], ts=opts["Input"]["indexCol"], nan_vals=opts["Output"]["outputNans"])
        
        logCurrentFile = "[" + os.path.split(fname)[1] + "]\t"

        #Apply QA actions
        d = applyQA(d, d_qa, d_dev)
        ...
        #Save QA stat data
        saveOutput(d_stats, opts["Output"]["outputPath"], fname_out, "_Level2_qa_stats", outputNans=opts["Output"]["outputNans"])
        ...
        d_out = avgToFreqs(d, timeCols=opts["Input"]["timeCols"], **opts["Level 2"])

        #Save base freq
        applyDataTypes(d, opts["Input"]["intCols"], opts["Input"]["timeCols"], opts["Output"]["outputNans"], opts["Output"]["outputDecs"], colDecs)
        saveOutput(d, opts["Output"]["outputPath"], fname_out, "_Level2", outputNans=opts["Output"]["outputNans"], index=False)
        ...
        for d_freq in d_out:
            ...
            #Save Level 2 data
            saveOutput(d_out[d_freq], opts["Output"]["outputPath"], fname_out, "_Level2_" + d_freq, outputNans=opts["Output"]["outputNans"], indexCol=opts["Input"]["indexCol"])
